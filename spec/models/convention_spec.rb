...
RSpec.describe Convention, type: :model do
  context  'create_default_fields' do
    ...
    it { expect(convention.convention_fields.count).to eql(4) }
    it { expect(convention.convention_fields.pluck(:codification_kind)).to\
      eq(['originating_company', 'discipline', 'document_type', 'document_number']) }
    it do
      convention2 = FactoryBot.create(:convention, number: 2)
      expect(convention2.convention_fields.pluck(:codification_kind)).to\
