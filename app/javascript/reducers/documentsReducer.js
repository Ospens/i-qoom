...
const documentsReducer = (state = initialState, action) => {
  ...
  case DOCUMENTS_FETCH_SUCCESS:
    return {
      ...
      originating_companies: action.payload.originating_companies.map(oc => ({ value: oc[0], title: oc[1], checked: true })),
      document_types: action.payload.document_types.map(dt => ({ value: dt[0], title: dt[1], checked: true })),
      discipline: action.payload.discipline.map(d => ({ value: d[0], title: d[1], checked: true })),
    }
  ...
  case TOGGLE_FILTERS:
    return {
      ...state,
      [action.payload.type]: state[action.payload.type].map(el => el.value === action.payload.value ? { ...el, checked: !el.checked } : el)
    }
