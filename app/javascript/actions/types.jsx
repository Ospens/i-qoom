/* User */
export const SIGN_IN_USER = 'SIGN_IN_USER'
export const SIGN_UP_USER = 'SIGN_UP_USER'
export const SIGN_OUT_USER = 'SIGN_OUT_USER'

/* Project */
export const EDIT_PROJECT = 'EDIT_PROJECT'
export const SET_PAGE_TITLE = 'SET_PAGE_TITLE'
export const PROJECT_CREATED_SUCCESS = 'PROJECT_CREATED_SUCCESS'
export const PROJECT_UPDATED_SUCCESS = 'PROJECT_UPDATED_SUCCESS'
export const PROJECT_CREATE_FAILURE = 'PROJECT_CREATE_FAILURE'
export const PROJECTS_FETCH_SUCCESS = 'PROJECTS_FETCH_SUCCESS'
export const PROJECT_FETCH_SUCCESS = 'PROJECT_FETCH_SUCCESS'
export const PROJECT_CODE_UPDATED = 'PROJECT_CODE_UPDATED'
export const PROJECTS_FETCH_FAILURE = 'PROJECTS_FETCH_FAILURE'
export const PROJECT_EXIT = 'PROJECT_EXIT'
export const PROJECT_ADMIN_DELETED = 'PROJECT_ADMIN_DELETED'
export const PROJECT_ADMIN_UPDATED = 'PROJECT_ADMIN_UPDATED'
export const TOGGLE_SIDEBAR = 'TOGGLE_SIDEBAR'
export const PROJECT_ADMINS_FETCHED = 'PROJECT_ADMINS_FETCHED'
export const CHECK_MAIN_SECTION_DMS = 'CHECK_MAIN_SECTION_DMS'

/* Admin Panel */
export const TRANSLATE_FETCH_SUCCESS = 'TRANSLATE_FETCH_SUCCESS'

/* Documents */
export const DOCUMENT_CREATE_SUCCESS = 'DOCUMENT_CREATE_SUCCESS'
export const DOCUMENT_FETCH_SUCCESS = 'DOCUMENT_FETCH_SUCCESS'
export const DOCUMENTS_FETCH_SUCCESS = 'DOCUMENTS_FETCH_SUCCESS'
export const CREATING_DOCUMENT = 'CREATING_DOCUMENT'
export const EDIT_DOCUMENT = 'EDIT_DOCUMENT'
export const REVISIONS_AND_VERSIONS_FETCH_SUCCESS = 'REVISIONS_AND_VERSIONS_FETCH_SUCCESS'
export const DOCUMENTS_FETCHED_WITHOUT_FILTERS_SUCCESS = 'DOCUMENTS_FETCHED_WITHOUT_FILTERS_SUCCESS'
export const TOGGLE_SEARCH_FILTERS = 'TOGGLE_SEARCH_FILTERS'
export const TOGGLE_FILTERS = 'TOGGLE_FILTERS'
export const TOGGLE_LOADING = 'TOGGLE_LOADING'
export const DMS_USERS_FETCHED = 'DMS_USERS_FETCHED'
export const DOCUMENTS_SORTED = 'DOCUMENTS_SORTED'

/* Conventions */
export const EDITING_CONVENTION = 'EDITING_CONVENTION'
export const CONVENTION_UPDATED = 'CONVENTION_UPDATED'

/* Planned list */
export const PLANNED_LISTS_FETCHED = 'PLANNED_LISTS_FETCHED'
export const PLANNED_LIST_FETCHED = 'PLANNED_LIST_FETCHED'
export const PLANNED_LIST_UPDATED = 'PLANNED_LIST_UPDATED'
export const PLANNED_LIST_ADDED = 'PLANNED_LIST_ADDED'
export const EDIT_PLANNED_LIST = 'EDIT_PLANNED_LIST'

/* Access rights */
export const GET_CURRENT_MEMBER = 'GET_CURRENT_MEMBER'
export const DELETE_TEAM = 'DELETE_TEAM'
export const GET_NEW_MEMBERS_LIST = 'GET_NEW_MEMBERS_LIST'
export const GET_CURRENT_MEMBERS_LIST = 'GET_CURRENT_MEMBERS_LIST'
export const UPDATE_TEAM_MEMBERS = 'UPDATE_TEAM_MEMBERS'
export const GET_CURRENT_TEAMS_LIST = 'GET_CURRENT_TEAMS_LIST'
export const GET_NEW_TEAMS_LIST = 'GET_NEW_TEAMS_LIST'
export const GET_OLD_TEAMS_LIST = 'GET_OLD_TEAMS_LIST'
export const UPDATE_NEW_TEAMS_LIST = 'UPDATE_NEW_TEAMS_LIST'
export const UPDATE_OLD_TEAMS_LIST = 'UPDATE_OLD_TEAMS_LIST'

/* Project members */
export const ACTIVE_MEMBERS_FETCHED_SUCCESS = 'ACTIVE_MEMBERS_FETCHED_SUCCESS'
export const PENDING_MEMBERS_FETCHED_SUCCESS = 'PENDING_MEMBERS_FETCHED_SUCCESS'
export const ACTIVE_MEMBERS_UPDATED = 'ACTIVE_MEMBERS_UPDATED'
export const PENDING_MEMBERS_UPDATED = 'PENDING_MEMBERS_UPDATED'
export const CREATING_PROJECT_MEMBER = 'CREATING_PROJECT_MEMBER'
export const PROJECT_MEMBER_CREATED = 'PROJECT_MEMBER_CREATED'

/* Disciplines */
export const DISCIPLINES_FETCHED = 'DISCIPLINES_FETCHED'
export const DISCIPLINES_UPDATED = 'DISCIPLINES_UPDATED'
export const DISCIPLINES_DELETED = 'DISCIPLINES_DELETED'
export const DISCIPLINE_CREATED = 'DISCIPLINE_CREATED'

/* Roles */
export const ROLES_FETCHED = 'ROLES_FETCHED'
export const ROLE_UPDATED = 'ROLES_UPDATED'
export const ROLE_DELETED = 'ROLES_DELETED'
export const ROLE_CREATED = 'ROLE_CREATED'

/* Folders */
export const FOLDER_CREATED = 'FOLDER_CREATED'
export const FOLDERS_FETCHED = 'FOLDERS_FETCHED'
export const FOLDER_UPDATED = 'FOLDER_UPDATED'
export const EDITING_FOLDER = 'EDITING_FOLDER'
export const DOCUMENT_ADDED = 'DOCUMENT_ADDED'

/* Notifications */
export const TOGGLE_STATE = 'TOGGLE_STATE'
export const ADD_NOTIFICATION = 'ADD_NOTIFICATION'
export const REMOVE_NOTIFICATION = 'REMOVE_NOTIFICATION'
